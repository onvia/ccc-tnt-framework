...
美术同学把UI位置摆好，我们只需要调整一下图层就可以了。  
大大提升了 UI 制作的效率  
相同的图像只会导出一张图片  
如果是相同的图像但是大小不同，则可以使用 `bind` 功能进行绑定，具体使用可以查看文档和示例文件
如果是相同的图像但是旋转角度不同，则会导出多张旋转角度不同的图片，需要手动去处理旋转

插件文件夹提供了一个能够正确导出为预制体的的 psd 文件可以作为参考  `extensions\psd2ui\test\demo.psd` 

> 本插件暂未上传 CocosCreator 商店
> 支持的 CocosCreator 版本 `2.4.x、3.4.2+`，仓库中的版本默认为 `3.4.2+` ，如果想用在 `2.4.x` 中，请修改 `extensions\psd2ui\dist\main.js` 的 `ENGINE_VER` 变量为 `v249`
...
## 仓库
gitee: https://gitee.com/onvia/ccc-tnt-framework  
github：https://github.com/onvia/ccc-tnt-framework  

插件工具在仓库 `extensions` 文件夹下